...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const updateCartCount = () => {
      ...
      const updatedTotalCount = updatedCart.reduce((total, item) => total + item.quantity, 0);
      setCartCount(updatedTotalCount);
    };
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between w-full gap-8">
        ...
        <div className="hidden md:flex items-center space-x-6   text-lg font-poppins">
          ...
          <button
            ...
            className=" hover:text-yellow-100 transition duration-200  text-blue-300 bg-white md:w-44"
          >
          ...
          <Link to="/cart" className="hover:text-yellow-100 text-lg">
            <div className="relative">
              <BsCart className="w-6 h-6" />
              {cartCount > 0 && (
                <span className="absolute -top-4 -right-4 bg-red-500 text-white text-[10px] px-2 rounded-full font-semibold">
                  {cartCount}
                </span>
              )}
            </div>
